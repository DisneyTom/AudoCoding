...
- (void)setWithCoder:(NSCoder *)aDecoder
{
    ...
    for (NSString *key in properties)
    {
        ...
        if (object)
        {
            ...
            {
                NSString * message = [NSString stringWithFormat:@"Expected '%@' to be a %@, but was actually a %@ in <%@>", key, propertyClass, [object class], self.class];
				NSLog(@"%@", message );
                [NSException raise:AutocodingException format:@"Expected '%@' to be a %@, but was actually a %@ in <%@>", key, propertyClass, [object class], self.class];
